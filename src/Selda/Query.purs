...
import Selda.Expr (Expr(..), _column)
...
crossJoin ∷ ∀ s r res v. FromTable s r res ⇒ Table r → Query s v { | res }
crossJoin table = do
...
groupBy ∷ ∀ s a v. Col s v a → Query s v (Aggr s v a)
...
orderBy ∷ ∀ s a v. Order → Col s v a → Query s v Unit
orderBy order (Col e) =
...
class FromTable s t c | s t → c where
  fromTable ∷ ∀ v. Table t → Query s v { res ∷ { | c } , sql ∷ (SQL v) }
...
class FromSubQuery s v inner res | s inner v → res where
  fromSubQuery
    ∷ Query (Inner s) v { | inner }
    → Query s v { res ∷ { | res } , sql ∷ (SQL v) , alias ∷ Alias }
...
instance subQueryResultInstance
    ...
    ⇒ MappingWithIndex SubQueryResult (SProxy sym) 
      (Col s v a) (Col s ( column ∷ Column a | v' ) a)
  where
  mappingWithIndex (SubQueryResult namespace) sym (Col _) = 
    Col $ inj _column $ Column { namespace, name: reflectSymbol sym }
